""" Sometimes we want to select data based on groups and understand aggregated data on a group level. We have seen that even though Pandas allows us to
iterate over every row in a dataframe, it is geneally very slow to do so. Fortunately Pandas has a groupby() function to speed up such task. The idea 
behind the groupby() function is that it takes some dataframe, splits it into chunks based on some key values, applies computation on those chunks, then 
combines the results back together into another dataframe. In pandas this is refered to as the split-apply-combine pattern.
 """
...
# Let's run such task for 3 times and time it. For this we'll use the cell magic function %%timei
...
transform_df['cancellation_policy'] = df['cancellation_policy']
transform_df.sort_values(by='cancellation_policy')
transform_df.head()
...
df.groupby('cancellation_policy_x').filter(
